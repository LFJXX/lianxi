...
NSString *const LFImagePickerDidSelectedNotification = @"LFImagePickerDidSelectedNotification";
NSString *const LFImagePickerDidSelectedAssetsKey = @"LFImagePickerDidSelectedAssetsKey";
NSString *const LFImagePickerBundleName = @"LFImagePicker.bundle";

/// 默认选择图像大小
#define LFImagePickerDefaultSize    CGSizeMake(600, 600)
...
@implementation LFImagePickerController {
    LFAlbumTableViewController *_rootViewController;
    /// 选中素材数组
...
- (instancetype)initWithSelectedAssets:(NSArray<PHAsset *> *)selectedAssets {
    ...
    if (self) {
        ...
        _rootViewController = [[LFAlbumTableViewController alloc] initWithSelectedAssets:_selectedAssets];
        // 默认最大选择图像数量
        ...
        [[NSNotificationCenter defaultCenter]
         ...
         name:LFImagePickerDidSelectedNotification
         object:nil];
    }
    return self;
}
...
- (void)dealloc {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:LFImagePickerDidSelectedNotification object:nil];
}

#pragma mark - getter & setter 方法
- (CGSize)targetSize {
    if (CGSizeEqualToSize(_targetSize, CGSizeZero)) {
        _targetSize = LFImagePickerDefaultSize;
    }
    return _targetSize;
}
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated {
    self.toolbarHidden = [viewController isKindOfClass:[LFAlbumTableViewController class]];
